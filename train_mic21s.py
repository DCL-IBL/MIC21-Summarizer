...
def load_checkpoint():
    model.projection_layer.load_state_dict(torch.load(f'model_{domain_name}_title_1.pth'))
    model.projection_norm.load_state_dict(torch.load(f'model_{domain_name}_title_2.pth'))
    return torch.load(f'model_{domain_name}_title_loss.pth')
